...
void ThrottleController::ThrottlePID(int32_t desiredValue) {
	if (desiredValue >= (speedCyclometerInput_mmPs + 10)) {
		noInterrupts();
    desiredSpeed_mmPs = desiredValue; // missing line?? added summer 2019
		speedPID.Compute();
		
    // engageThrottle(PIDThrottleOutput_pwm); // instead of original code which calling a function that is not for PID output
    //using following (added summer 2019)
    if(PIDThrottleOutput_pwm != currentThrottlePWM){
    Serial.println(desiredValue);
    write(DAC_CHANNEL, PIDThrottleOutput_pwm);
    currentThrottlePWM = PIDThrottleOutput_pwm;
    }
		interrupts();
	}
}
...
void ThrottleController::engageThrottle(int32_t input) {
  ...
  if (input != 0){
    ...
    // updated version (summer 2019)
    
    if(input > speedCyclometerInput_mmPs){
      input = map(input - speedCyclometerInput_mmPs, 0, MAX_SPEED_mmPs, MIN_ACC_OUT, MAX_ACC_OUT);
    }
    else{
      input = MIN_ACC_OUT;
      }
